...
function transformUrl(url) {
  return url.replace(
    ["http", "://", "mpegmedia", ".abc.net.au"].join(""),
    "https://abcmedia.akamaized.net"
  );
}
...
function Lively(node) {
  ...
  if (win.livelyMetadata && win.livelyMetadata[videoId]) {
    ...
    $.getJSON(
      `https://content-gateway.abc-prod.net.au/api/v1/content/id/${videoId}`
    ).done(initVid);
  }

  function initVid(json) {
    var $inst;
    json.renditions.forEach(r => {
      r.url = transformUrl(r.url);
    });
    $inst = $(
      template({
        renditions: json.renditions.sort(
          (a, b) => (a.bitrate > b.bitrate ? -1 : a.bitrate < b.bitrate ? 1 : 0)
        ),
        poster: json.relatedItems.length
          ? cmimg(
              json.relatedItems[0].id,
              "16x9",
              $video.width() * win.devicePixelRatio
            )
          : false,
...
Lively.prototype.updatePosition = function() {
  ...
  this.visible =
    this.position.top -
      (viewport.top - viewport.bottom) * this.settings.scrollplayPosition <
      viewport.bottom &&
